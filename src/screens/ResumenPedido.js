...
const ResumenPedido = ({
    ...
    const realizarPago = () => {
        ...
        // Aquí podrías añadir una llamada a Firebase si quisieras registrar el pago
    };
    ...
    return (
        <div className="p-4">
            ...
            {/* Rondas confirmadas */}
            {rondasConfirmadas.length > 0 && (
                <div className="mb-4">
                    <h3 className="font-semibold mb-1">🧾 Rondas Confirmadas:</h3>
                    {rondasConfirmadas.map((ronda, i) => (
                        <div key={i} className="mb-2 bg-gray-100 p-2 rounded-md border">
                            <p className="text-sm font-medium text-[#BB5030] mb-1">Ronda {i + 1}</p>
                            {ronda.map((item, idx) => (
                                <div
                                    key={idx}
                                    className="text-sm flex justify-between text-gray-700"
                                >
                                    <span>{item.cantidad > 1 && `${item.cantidad} x `}{item.producto} {item.tipo && `(${item.tipo})`}</span>
                                    <span>{(parseFloat(item.precio?.replace('€', '').replace(',', '.') || 0) * (item.cantidad || 1)).toFixed(2)} €</span>
                                </div>
                            ))}
                        </div>
                    ))}
            ...
            {/* Total y acciones */}
            {(rondaActual.length > 0 || rondasConfirmadas.length > 0) && (
